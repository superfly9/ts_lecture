...
let v1 : typeof bb = 'good awesome'; // Error : bb를 타입으로 할당했기에 'ES2020'값만 쓸 수 있다
let v2 : typeof bb = 'ES2020'; // ok
let v3:string =bb; // ok  
...
const arr2:Array<number | string> = [10,20,30]; // number|string []
...
interface Korea extends Country {
    awesome : boolean;
}
...
const noname:Country = {name:'nop',location:'dont know',history:0};
const korea:Korea = { name : 'Korea', location:'center of the world',history : 5000,awesome:true};
const japan:Japan = { name:'?',location:'?',history:0,fucking:true};
...
function foo3 (a:number | string = 'seoul', b : number) :string {
    return `${a} : ${b}`
}
